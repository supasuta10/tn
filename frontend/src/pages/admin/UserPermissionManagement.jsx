...
import { Search, Filter, Plus, Eye, Edit, Trash2, UserCheck, UserX, User, ChefHat, Users } from 'lucide-react';
...
const UserPermissionManagement = () => {
  ...
  const toggleUserStatus = async (user) => {
    try {
      const currentStatus = user.isActive !== false ? 'ใช้งานอยู่' : 'ไม่ใช้งาน';
      const newStatus = user.isActive !== false ? 'ไม่ใช้งาน' : 'ใช้งานอยู่';

      const result = await Swal.fire({
        title: 'คุณแน่ใจหรือไม่?',
        text: `คุณต้องการ${user.isActive !== false ? 'ปิด' : 'เปิด'}การใช้งานของ ${user.title}${user.firstName} ${user.lastName} ใช่หรือไม่? ปัจจุบัน: ${currentStatus} → ${newStatus}`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: user.isActive !== false ? '#d33' : '#3085d6',
        cancelButtonColor: '#6b7280',
        confirmButtonText: user.isActive !== false ? 'ใช่, ปิดการใช้งาน!' : 'ใช่, เปิดการใช้งาน!',
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          icon: 'success',
          title: user.isActive !== false ? 'ปิดการใช้งานแล้ว!' : 'เปิดการใช้งานแล้ว!',
          text: `สถานะการใช้งานของ ${user.title}${user.firstName} ${user.lastName} ได้ถูก${user.isActive !== false ? 'ปิด' : 'เปิด'}เรียบร้อยแล้ว`,
          confirmButtonColor: user.isActive !== false ? '#22c55e' : '#22c55e'
        });
      }
    } catch (err) {
  ...
  const toggleUserRole = async (user, newRole) => {
    try {
      ...
      const result = await Swal.fire({
        title: 'คุณแน่ใจหรือไม่?',
        text: `คุณต้องการเปลี่ยนบทบาทของ ${user.title}${user.firstName} ${user.lastName} จาก ${currentRoleName} เป็น ${newRoleName} ใช่หรือไม่?`,
        icon: 'warning',
        showCancelButton: true,
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'เปลี่ยนบทบาทแล้ว!',
          text: `บทบาทของ ${user.title}${user.firstName} ${user.lastName} ได้ถูกเปลี่ยนจาก ${currentRoleName} เป็น ${newRoleName} เรียบร้อยแล้ว`,
      ...
      Swal.fire({
        icon: 'error',
        title: 'การเปลี่ยนบทบาทล้มเหลว',
        text: err.response?.data?.message || 'เกิดข้อผิดพลาดขณะเปลี่ยนบทบาทผู้ใช้งาน',
        confirmButtonColor: '#dc2626'
      });
    }
  };
  ...
  const addUser = async () => {
    ...
    const { value: formValues } = await Swal.fire({
      ...
      preConfirm: () => {
        ...
        if (!title || !firstName || !lastName || !email || !phone || !username || !password || !role) {
    ...
    if (formValues) {
      try {
        ...
        Swal.fire({
          ...
          text: `ผู้ใช้งาน ${formValues.title}${formValues.firstName} ${formValues.lastName} ได้ถูกเพิ่มเรียบร้อยแล้ว`,
          confirmButtonColor: '#22c55e'
        });
      } catch (err) {
        ...
        Swal.fire({
          icon: 'error',
          title: 'การเพิ่มผู้ใช้งานล้มเหลว',
          text: err.response?.data?.message || 'เกิดข้อผิดพลาดขณะเพิ่มผู้ใช้งานใหม่',
          confirmButtonColor: '#dc2626'
        });
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">จัดการสิทธิ์ผู้ใช้งาน</h1>
          <p className="text-gray-600">จัดการบทบาทและสิทธิ์การใช้งานของผู้ใช้งานทั้งหมดในระบบ</p>
        </div>
        <button
          onClick={addUser}
                ...
                <tr key={user._id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-medium text-sm ${
                        ...
                        user.role === 'chef' ? 'bg-gradient-to-r from-orange-500 to-red-500' :
                        user.role === 'staff' ? 'bg-gradient-to-r from-blue-500 to-blue-600' :
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <select
                      ...
                      className={`text-xs font-semibold rounded-full px-2 py-1 ${getRoleColor(user.role)} border-0 focus:ring-2 focus:ring-blue-500`}
                    >
                      <option value="admin">ผู้ดูแลระบบ</option>
                      <option value="chef">พ่อครัว</option>
                      <option value="staff">เจ้าหน้าที่</option>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(user.isActive)}`}>
                      {user.isActive !== false ? 'ใช้งานอยู่' : 'ไม่ใช้งาน'}
                    </span>
                  </td>
